...
const Tab = ({ day = "Day 01", date = "18 April", active = false, handleActive }) => {
    return (
        <div onClick={handleActive} className={`border-b-4 ${active ? "border-[#381DDB]" : "border-[#dfdfdf]"} pb-4 flex-[0.3] cursor-pointer`}>
            <div className={`${active ? "text-[#381DDB]" : "text-[#98989a]"} text-[24px] font-extrabold leading-[38px]`}>{day}</div>
            <div className={`${active ? "text-[#381DDB]" : "text-[#d1d0d6]"} text-[16px] font-bold leading-[19.5px]`}>{date}</div>
        </div>
    )
}